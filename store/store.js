...
const getters = {
  ...
  productosYaComprados: state =>
    ...
    prodByCatePendientes: (state) => (cate) => {
        return state.listaProductos.filter(prod => (prod.cate === parseInt(cate)) && prod.cant).length
      },
}
...
const mutations = {
  ...
  DEL_PROD (state, prod) {
    debugger
    var index = state.listaProductos.indexOf(prod);
    state.listaProductos.slice(index,1);
  },
  GET_PRODS (state) {
    state.loading = true
    axios
